...
func getRandomNumber(n *big.Int) *big.Int {
...
// Paillier Encryption: Encrypt the vote (YES = 1, NO = 0)
func EncryptPaillier(vote int, n *big.Int, g *big.Int) *big.Int {
    r := getRandomNumber(n) // Choose a random r
    nSquare := new(big.Int).Mul(n, n)
 
    // Compute ciphertext: c = (g^vote * r^n) mod n²
    c1 := new(big.Int).Exp(g, big.NewInt(int64(vote)), nSquare)
    c2 := new(big.Int).Exp(r, n, nSquare)
...
// func logVote(txID, mspID string, vote int) {
//  // fabricSamplesPath := os.Getenv("FABRIC_SAMPLES_PATH")
//  // if fabricSamplesPath == "" {
//  //  fabricSamplesPath = "/path/to/fabric-samples" // Set a default path
//  // }
//  // filename := filepath.Join(fabricSamplesPath, "test-network", "votes.log")
 
//  filename := "$HOME/fabric-samples/test-network/land-registration-system/votes.log"
//  // homeDir, _ := os.UserHomeDir()
//  // filename := filepath.Join(homeDir, "fabric-samples", "test-network", "land-registration-system", "votes.log")
//     file, err := os.OpenFile(filename, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
//     if err != nil {
//      endorserLogger.Errorf("Failed to open vote log file: %v", err)
//         return
//     }
//     defer file.Close()
...
//  if err != nil {
//      endorserLogger.Errorf("Failed to write to vote log: %v", err)
//  } else {
//      endorserLogger.Infof("Vote logged successfully: %s", txID)
//  }
// }
...
func (e *Endorser) ProcessProposal(ctx context.Context, signedProp *pb.SignedProposal) (*pb.ProposalResponse, error) {
    ...
    // Extract the chaincode function name
    functionName := ""
    if up.Input != nil {
        if len(up.Input.Args) > 0 {
            functionName = string(up.Input.Args[0]) // First argument is the function name
        }
    }
 
    // Process proposal successfully
    ...
    // Introduce randomness only for CreateAsset or TransferAsset
    if functionName == "CreateAsset" || functionName == "SellAsset" {
    
        err := fmt.Errorf("random failure for testing")
        endorserLogger.Warnw("Random failure injected", "channel", up.ChannelHeader.ChannelId, "chaincode", up.ChaincodeName, "function", functionName, "error", err.Error())
        ...
        g.SetString("",10)
        ...
        // var mspID string = "unknown"
 
        mspInstance, ok := e.LocalMSP.(msp.MSP)
        orgNum := 1
        if !ok {
            endorserLogger.Error("LocalMSP is not a full MSP implementation")
            // return nil, fmt.Errorf("MSP interface assertion failed")
        } else {
            mspID, err := mspInstance.GetIdentifier()
            if err != nil {
                endorserLogger.Errorw("Failed to get MSP ID", "error", err)
                // return nil, fmt.Errorf("MSP ID retrieval error: %v", err)
            } else {
                re := regexp.MustCompile(`\d+`)
                orgNumberStr := re.FindString(mspID) // Extract number as string
 
                orgNumber, err := strconv.Atoi(orgNumberStr) // Convert to int
                if err != nil {
                    endorserLogger.Errorw("Failed to convert org number", "error", err)
                }
                orgNum = orgNumber
                endorserLogger.Infof(orgNumberStr)
 
            }
        }
 
        // Modify response to include 'yes' or 'no' based on probability
        randomResponse := q // Default response
    ...
    endorserLogger.Warnw("Random failure injected", "response", pResp.Response)
